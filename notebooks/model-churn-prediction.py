...
# MAGIC %md
# MAGIC # Churn model - Training a simple `Sklearn` classification model
...
# MAGIC * Lauch a cluster with Databricks Runtime 6.5 ML or above
# MAGIC * Have a ADLS (Azure DataLake Storage) configured and mounted (see this [tutorial](https://github.com/lfbraz/azure-databricks/blob/master/notebooks/read-from-adls.ipynb))
# MAGIC 
...
import requests
filename = 'dados_clientes.csv'

url = "https://raw.githubusercontent.com/lfbraz/azure-databricks-mlops/master/dataset/{}".format(filename)
output_local_file_path = "/tmp/{}".format(filename)
output_dbfs_file_path = "/dataset/{}".format(filename)

print('Downloading from: {} to DBFS: {}'.format(url, output_local_file_path))
...
print('File transfered from: {} to dbfs {}'.format(output_local_file_path, output_dbfs_file_path))

# COMMAND ----------

# MAGIC %md
# MAGIC Now we can import the file to a `pandas dataframe`

# COMMAND ----------

import pandas as pd

customer_data = pd.read_csv("/dbfs/dataset/{}".format(filename), sep=',')
...
# MAGIC %md Plot a histogram of the dependent variable, **IndiceSatisfacao** (An index that measure how satisfied a customer is with the service).
...
# MAGIC %md
# MAGIC We can see some columns with missing values, now let's remove these rows (we can use another strategy as well, but here we will remove all these rows.)
...
X_train = train[numeric_columns].drop([TARGET_COLUMN, 'CodigoCliente'], axis=1)
X_test = test[numeric_columns].drop([TARGET_COLUMN, 'CodigoCliente'], axis=1)
...
# MAGIC %md
# MAGIC Calculate AUC and ROC Curve
...
query = "tags.mlflow.runName='{}'".format(run_name)
...
# MAGIC %md
# MAGIC Now we can package this model to be deployed ([see]())